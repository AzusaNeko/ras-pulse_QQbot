...
describe('parseQQKeywordCommand', () => {
  it('parses add, remove, and list commands', () => {
    ...
    expect(parseQQKeywordCommand('/bind Azusa')).toEqual({ type: 'bind', name: 'Azusa' })
    ...
    expect(parseQQKeywordCommand('/add バンドリ exclude バンドリエール、バンドリング')).toEqual({ type: 'add', keyword: 'バンドリ', excludeKeywords: ['バンドリエール', 'バンドリング'] })
    ...
    expect(parseQQKeywordCommand('/remove Switch')).toEqual({ type: 'remove', keyword: 'Switch' })
    expect(parseQQKeywordCommand('关键词列表')).toEqual({ type: 'list' })
    expect(parseQQKeywordCommand('/list')).toEqual({ type: 'list' })
    ...
    expect(parseQQKeywordCommand('/clear confirm')).toEqual({ type: 'clear', confirmed: true })
  })
})
