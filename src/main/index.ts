...
function showImageToast(item: MercariItem, settings: AppSettings, imageUrl: string | undefined, isTest = false): void {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><style>
    *{box-sizing:border-box}html,body{margin:0;width:100%;height:100%;overflow:hidden;background:transparent;font-family:"Segoe UI","Microsoft YaHei",sans-serif}
    .toast{width:390px;height:${height}px;display:flex;gap:13px;padding:11px;color:#eaf5ef;background:linear-gradient(135deg,#10271c,#0b1712 72%);border:1px solid #397454;border-radius:14px;box-shadow:0 14px 42px #000b;cursor:pointer}
    .media{width:116px;height:116px;flex:none;display:grid;place-items:center;border-radius:9px;overflow:hidden;background:#193325;color:#8ba99a;font-size:11px}.media img{display:block;width:100%;height:100%;object-fit:cover}.copy{min-width:0;display:flex;flex:1;flex-direction:column;padding:2px 4px 2px 0}.tag{color:#6df0ac;font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.name{margin-top:9px;line-height:1.35;font-size:13px;font-weight:600;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}.bottom{display:flex;align-items:end;justify-content:space-between;gap:8px;margin-top:auto}.price{color:#7df4b5;font-family:monospace;font-size:15px}.hint{color:#749085;font-size:10px}
  </style></head><body><div class="toast" onclick="window.open('${url}','_blank')">${media}<div class="copy"><div class="tag">${title}</div><div class="name">${name}</div><div class="bottom"><b class="price">${priceText}</b><span class="hint">点击打开商品 ↗</span></div></div></div>${loader}</body></html>`
